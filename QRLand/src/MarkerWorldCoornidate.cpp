...
MarkerConfig::MarkerConfig(int _id, int _boardsize, float _x, float _y, float _z)
{
	id = _id;
...
std::ostream &operator<<(std::ostream &os, const MarkerConfig & mc)
{
	os << "id=" << mc.id << std::endl;
	os << "size=" << mc.boardsize << std::endl;
	os << "coordinate=" << mc.coordinate << std::endl;
