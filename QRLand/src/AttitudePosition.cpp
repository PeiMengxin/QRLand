		...
		cv::Point3f &pos_camera, Attitude &atti_camera, int flag /*= 0*/)
{
	...
	case 0:
	{
		for (unsigned int i = 0; i < Markers.size(); i++)
		{
			...
			pos_camera += ((markermap.find(Markers[i].id))->second).coordinate + pos_world;
...
void getAttitude(aruco::Marker marker, Attitude &attitude)
{
	...
	attitude.Pit = atan2(2 * (q0 * q1 + q2 * q3), -1 + 2 * (q1 * q1 + q2 * q2))
			* 57.3f;
	attitude.Rol = asin(2 * (q1 * q3 - q0 * q2)) * 57.3f;
	attitude.Yaw = -atan2(2 * (-q1 * q2 - q0 * q3), 1 - 2 * (q0 * q0 + q1 * q1))
			* 57.3f;
}
